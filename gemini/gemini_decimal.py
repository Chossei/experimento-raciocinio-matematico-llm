...
log_filename = os.path.join(LOGS_DIR, f'batch_decimais_{data_hora}.log')
...
logging.info("=== Iniciando Preparação de Lotes DECIMAIS (Batch) para Gemini Vertex AI ===")
...
def extrair_numero_decimal(texto):
    ...
    Retorna o número como string para manter a precisão exata
    e permitir comparação direta com o Resultado_original_decimal.
    """
    texto_limpo = texto.strip()
    # Tenta encontrar um número decimal (ex: 12.193263111263526)
...
def main():
    ...
    # Verificar se as colunas decimais existem no CSV
    if 'Conta_decimal' not in df_operacoes.columns:
        logging.error("A coluna 'Conta_decimal' não existe no arquivo de operações.")
        logging.error("Execute novamente o pipeline para gerar as colunas decimais.")
        return

    # Determinar quais operações já foram feitas
    concluidos = set()
    if os.path.exists(ARQUIVO_RESULTADOS_DEC):
        df_resultados = pd.read_csv(ARQUIVO_RESULTADOS_DEC)
    ...
    for modelo, reqs in pendencias_por_modelo.items():
        if not reqs:
            logging.info(f"Modelo {modelo} já completou todas as operações decimais. Pulando.")
            continue

        logging.info(f"Preparando lote DECIMAL de {len(reqs)} requisições para {modelo}...")

        # Salvar arquivo JSONL localmente
        jsonl_filename = f"pedidos_decimais_{modelo}.jsonl"
        ...
        mapeamento_filename = f"mapa_decimais_{modelo}.json"
        mapeamento_path = os.path.join(PASTA_GEMINI, mapeamento_filename)
        mapa = []
        for r in reqs:
            mapa.append({
                "chave": r["chave"],
                "conta_decimal": r["conta_decimal"],
                "operacao": r["operacao"],
                "resultado_original_decimal": r["resultado_original_decimal"]
            })
        ...
        dest_uri = f"gs://{NOME_BUCKET}/outputs_decimais/{modelo}/"

        try:
            logging.info(f"Submetendo Job Batch DECIMAL para {modelo}...")
            ...
            logging.info(f"Job DECIMAL criado com sucesso! ID do Job: {job_name}")

        except Exception as e:
            logging.error(f"Erro ao submeter job decimal para {modelo}: {e}")

    # Salvar registro de jobs
    with open(ARQUIVO_JOBS, 'w', encoding='utf-8') as f:
        json.dump(jobs_criados, f, indent=4, ensure_ascii=False)

    logging.info(f"Total de jobs registrados (inteiros + decimais): {len(jobs_criados)}")
    logging.info("Submissão DECIMAL concluída! Use o verificador_batch.py para acompanhar o status.")
